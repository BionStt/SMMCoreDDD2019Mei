...
{
    public class GetDataPenjualanHarianQueryHandler : IRequestHandler<GetDataPenjualanHarianQuery, GetDataPenjualanHarianViewModel>
    {
        private readonly ISMMCoreDDD2019DbContext _context;
        private readonly IMapper _mapper;
        public GetDataPenjualanHarianQueryHandler(ISMMCoreDDD2019DbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        public async Task<GetDataPenjualanHarianViewModel> Handle(GetDataPenjualanHarianQuery request, CancellationToken cancellationToken)
        {
            ...
            var model = new GetDataPenjualanHarianViewModel
            {
                DataPenjualanHarianDS = _mapper.Map<IEnumerable<GetDataPenjualanHarianLookUpModel>>(aa)
            };
            return model;
        }
    }
