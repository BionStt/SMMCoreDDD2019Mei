...
{
    public class GetLaporanLabaRugiQueryHandler : IRequestHandler<GetLaporanLabaRugiQuery, GetLaporanLabaRugiViewModel>
    {
        private readonly ISMMCoreDDD2019DbContext _context;
        private readonly IMapper _mapper;

        public GetLaporanLabaRugiQueryHandler(ISMMCoreDDD2019DbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        public async Task<GetLaporanLabaRugiViewModel> Handle(GetLaporanLabaRugiQuery request, CancellationToken cancellationToken)
        {
            ...
            var model = new GetLaporanLabaRugiViewModel
            {
                LaporanLabaRugiDS = _mapper.Map<IEnumerable<GetLaporanLabaRugiLookUpModel>>(aa)
            };
            return model;
        }
    }
