...
namespace SmmCoreDDD2019.Application.PenjualanDetails.Query.GetDataCekDPBulanan
{
    public class GetDataCekDPBulananQueryHandler : IRequestHandler<GetDataCekDPBulananQuery, GetDataCekDPBulananViewModel>
    {
        ...
        public async Task<GetDataCekDPBulananViewModel> Handle(GetDataCekDPBulananQuery request, CancellationToken cancellationToken)
        {
            var aa = await (from a in _context.Penjualan
                            ...
                            where  ((a.TanggalPenjualan.Value <= request.TanggalPenjualan2 && a.TanggalPenjualan.Value >= request.TanggalPenjualan1) || (a.TanggalPenjualan.Value <= request.TanggalPenjualan2a && a.TanggalPenjualan.Value >= request.TanggalPenjualan1a)) 
                            select new
                            {
            ...
            var model = new GetDataCekDPBulananViewModel
            {
                CekDataCekDPBulananDS = _mapper.Map<IEnumerable<GetDataCekDPBulananLookUpModel>>(aa)
            };
            return model;
